...
func TestPusherAppendable(t *testing.T) {
	pusher := &fakePusher{}
	pa := NewPusherAppendable(pusher, "user-1", "instance-1", nil, prometheus.NewCounter(prometheus.CounterOpts{}), prometheus.NewCounter(prometheus.CounterOpts{}))
...
func TestPusherErrors(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			pa := NewPusherAppendable(pusher, "user-1", "instance-1", ruleLimits{evalDelay: 10 * time.Second}, writes, failures)
