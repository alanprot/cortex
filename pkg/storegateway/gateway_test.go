...
func TestStoreGateway_SeriesQueryingShouldEnforceMaxChunksPerQueryLimit(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"should return error if the actual number of queried chunks is > limit": {
			...
			expectedErr: httpgrpc.Errorf(http.StatusUnprocessableEntity, fmt.Sprintf("exceeded chunks limit: limit %d violated (got %d)", chunksQueried-1, chunksQueried)),
		},
	}
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			if testData.expectedErr != nil {
				require.Error(t, err)
				assert.IsType(t, testData.expectedErr, errors.Cause(err))
				s1, ok := status.FromError(errors.Cause(err))
				assert.True(t, ok)
				s2, ok := status.FromError(errors.Cause(testData.expectedErr))
				assert.True(t, ok)
				assert.True(t, strings.Contains(s2.Message(), s1.Message()))
